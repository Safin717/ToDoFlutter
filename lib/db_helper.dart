...
class SQLHelper{
  ...
  static Future<int> createData(String title, String? desc, bool isChecked)
  async {
    final db = await SQLHelper.db();
    final data = {'title':title, 'desc':desc, 'isChecked': isChecked? 1:0};
  ...
  static Future<int> updateData(int id, String title, String? desc, bool isChecked)
  async {
    final db = await SQLHelper.db();
    final data = {'title': title, 'desc': desc, 'isChecked': isChecked? 1:0, 'createdAt': DateTime.now().toString()};
    final result = await db.update('data', data, where: "id = ?", whereArgs: [id]);

    return result;
  }

  static Future<void> deleteData(int id)
  async {
    final db = await SQLHelper.db();
    try {
